...
// ============ PHASE 16: AI-POWERED TUTORING & STUDY PLANNING ============

enum HintLevel { basic, intermediate, detailed, fullExplanation }

enum FeedbackType { positive, corrective, progressive, motivational }

enum TutoringSessionStatus { scheduled, active, completed, cancelled, paused }

enum TutoringType { oneOnOne, groupSession, asynchronous, interactive }

enum StudyPlanFrequency { daily, threeDaysWeek, fivePerWeek, weeklyReview }

enum OptimizationStrategy { aggressive, balanced, gradual, conservative }

/// Adaptive study plan generated by AI
class AdaptiveStudyPlan {
  ...
  int get daysRemaining => targetExamDate.difference(DateTime.now()).inDays;
  bool get isOnTrack => sessions.where((s) => s.completed).length >= sessions.length * 0.8;
  ...
  factory AdaptiveStudyPlan.fromMap(Map<String, dynamic> map) {
    return AdaptiveStudyPlan(
      id: map['id'] ?? '',
      studentId: map['studentId'] ?? '',
      startDate: map['startDate'] != null ? DateTime.parse(map['startDate']) : DateTime.now(),
      targetExamDate: map['targetExamDate'] != null ? DateTime.parse(map['targetExamDate']) : DateTime.now(),
      sessions: (map['sessions'] as List<dynamic>?)?.map((s) => StudySession.fromMap(s as Map<String, dynamic>)).toList() ?? [],
      priorityCategories: List<String>.from(map['priorityCategories'] ?? []),
      estimatedTotalHours: map['estimatedTotalHours'] ?? 0,
      frequency: _parseStudyPlanFrequency(map['frequency'] ?? 'daily'),
      strategy: _parseOptimizationStrategy(map['strategy'] ?? 'balanced'),
...
/// Individual study session in adaptive plan
class StudySession {
  final String id;
  final String category;
  ...
  factory StudySession.fromMap(Map<String, dynamic> map) {
    return StudySession(
      ...
      durationMinutes: map['durationMinutes'] ?? 0,
      topicsToReview: List<String>.from(map['topicsToReview'] ?? []),
      questionIds: List<String>.from(map['questionIds'] ?? []),
...
class PersonalizedTutorSession {
  final String id;
  final String studentId;
  ...
  final List<TutoringInteraction> interactions;
  final double sessionProductivity; // 0.0-1.0
  final String? feedbackProvided;
  final List<String> nextRecommendedTopics;

  PersonalizedTutorSession({
    required this.id,
    required this.studentId,
    ...
    required this.sessionProductivity,
    this.feedbackProvided,
    required this.nextRecommendedTopics,
  });

  bool get isActive => status == TutoringSessionStatus.active;
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'sessionProductivity': sessionProductivity,
      'feedbackProvided': feedbackProvided,
      'nextRecommendedTopics': nextRecommendedTopics,
    };
  }

  factory PersonalizedTutorSession.fromMap(Map<String, dynamic> map) {
    return PersonalizedTutorSession(
      id: map['id'] ?? '',
      studentId: map['studentId'] ?? '',
      tutoringType: map['tutoringType'] ?? '',
      category: map['category'] ?? '',
      focusTopics: List<String>.from(map['focusTopics'] ?? []),
      startTime: map['startTime'] != null ? DateTime.parse(map['startTime']) : DateTime.now(),
      endTime: map['endTime'] != null ? DateTime.parse(map['endTime']) : null,
      status: _parseTutoringSessionStatus(map['status'] ?? 'scheduled'),
      sessionDurationMinutes: map['sessionDurationMinutes'] ?? 0,
      interactions: (map['interactions'] as List<dynamic>?)?.map((i) => TutoringInteraction.fromMap(i as Map<String, dynamic>)).toList() ?? [],
      sessionProductivity: (map['sessionProductivity'] ?? 0.5).toDouble(),
      feedbackProvided: map['feedbackProvided'],
      nextRecommendedTopics: List<String>.from(map['nextRecommendedTopics'] ?? []),
    );
  }
}

/// Tutoring interaction (Q&A exchange)
class TutoringInteraction {
  final String id;
  final String questionId;
  ...
  final double learningGain; // 0.0-1.0
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'hintLevel': hintLevel.toString().split('.').last,
  ...
  factory TutoringInteraction.fromMap(Map<String, dynamic> map) {
    return TutoringInteraction(
      id: map['id'] ?? '',
      questionId: map['questionId'] ?? '',
      studentResponse: map['studentResponse'] ?? '',
      tutorFeedback: map['tutorFeedback'] ?? '',
      hintLevel: _parseHintLevel(map['hintLevel'] ?? 'basic'),
      feedbackType: _parseFeedbackType(map['feedbackType'] ?? 'progressive'),
      isCorrect: map['isCorrect'] ?? false,
      timestamp: map['timestamp'] != null ? DateTime.parse(map['timestamp']) : DateTime.now(),
      learningGain: (map['learningGain'] ?? 0.0).toDouble(),
    );
  }
}
...
class HintProgression {
  final String id;
  final String questionId;
  final List<String> hintsByLevel; // [basic, intermediate, detailed, fullExplanation]
  final List<int> hintAccessTimes; // milliseconds since question start
  final int maxHintsAllowed;
  final bool revealedFull;

  HintProgression({
    required this.id,
    required this.questionId,
    required this.hintsByLevel,
    required this.hintAccessTimes,
    required this.maxHintsAllowed,
    required this.revealedFull,
  });

  String getNextHint() {
    final nextLevel = hintAccessTimes.length;
    if (nextLevel < hintsByLevel.length && nextLevel < maxHintsAllowed) {
      return hintsByLevel[nextLevel];
    }
    return hintsByLevel.last;
  }
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'hintAccessTimes': hintAccessTimes,
      'maxHintsAllowed': maxHintsAllowed,
      'revealedFull': revealedFull,
    };
  }

  factory HintProgression.fromMap(Map<String, dynamic> map) {
    return HintProgression(
      id: map['id'] ?? '',
      questionId: map['questionId'] ?? '',
      hintsByLevel: List<String>.from(map['hintsByLevel'] ?? []),
      hintAccessTimes: List<int>.from(map['hintAccessTimes'] ?? []),
      maxHintsAllowed: map['maxHintsAllowed'] ?? 3,
      revealedFull: map['revealedFull'] ?? false,
    );
  }
}
...
class PerformanceFeedback {
  ...
  final double accuracyImprovement; // -100.0 to 100.0
  final int correctAnswers;
  final int totalAttempts;
  ...
  double get accuracyRate => totalAttempts > 0 ? (correctAnswers / totalAttempts) * 100 : 0.0;
  ...
  factory PerformanceFeedback.fromMap(Map<String, dynamic> map) {
    return PerformanceFeedback(
      id: map['id'] ?? '',
      studentId: map['studentId'] ?? '',
      sessionId: map['sessionId'] ?? '',
      accuracyImprovement: (map['accuracyImprovement'] ?? 0.0).toDouble(),
      correctAnswers: map['correctAnswers'] ?? 0,
      ...
      suggestedNextSteps: List<String>.from(map['suggestedNextSteps'] ?? []),
      generatedAt: map['generatedAt'] != null ? DateTime.parse(map['generatedAt']) : DateTime.now(),
    );
  }
}

/// Helper functions for Phase 16 enum parsing
HintLevel _parseHintLevel(String value) {
  try {
    return HintLevel.values.firstWhere((e) => e.toString().split('.').last == value);
  } catch (e) {
    return HintLevel.basic;
  }
}

FeedbackType _parseFeedbackType(String value) {
  try {
    return FeedbackType.values.firstWhere((e) => e.toString().split('.').last == value);
  } catch (e) {
    return FeedbackType.progressive;
  }
}

TutoringSessionStatus _parseTutoringSessionStatus(String value) {
  try {
    return TutoringSessionStatus.values.firstWhere((e) => e.toString().split('.').last == value);
  } catch (e) {
    return TutoringSessionStatus.scheduled;
  }
}

StudyPlanFrequency _parseStudyPlanFrequency(String value) {
  try {
    return StudyPlanFrequency.values.firstWhere((e) => e.toString().split('.').last == value);
  } catch (e) {
    return StudyPlanFrequency.daily;
  }
}

OptimizationStrategy _parseOptimizationStrategy(String value) {
  try {
    return OptimizationStrategy.values.firstWhere((e) => e.toString().split('.').last == value);
  } catch (e) {
